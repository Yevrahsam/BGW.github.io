...
function update2(){
    ...
    // computer plays for itself, and we must be able to beat it
    // simple AI
    com.y += ((ball2.y - (com.y + com.height/2)))*0.07;
    ...
    // we check if the paddle hit the user or the com paddle
    let player = (ball2.x + ball2.radius < canvas.width/2) ? user : com;
    
    // if the ball hits a paddle
    if(collision(ball2,player)){
        // play sound
        hit.play();
        // we check where the ball hits the paddle
        let collidePoint = (ball2.y - (player.y + player.height/2));
        // normalize the value of collidePoint, we need to get numbers between -1 and 1.
        // -player.height/2 < collide Point < player.height/2
        collidePoint = collidePoint / (player.height/2);
        ...
        // change the X and Y velocity direction
        let direction = (ball2.x + ball2.radius < canvas.width/2) ? 1 : -1;
        ball2.velocityX = direction * ball2.speed * Math.cos(angleRad);
        ball2.velocityY = ball2.speed * Math.sin(angleRad);
